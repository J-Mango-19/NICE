...
class ResNetBlock(nn.Module):
    ...
    def forward(self, x):
        # x should be NxCx32x32 for CIFAR10
        identity = x
...
class ResNet(nn.Module):
    def __init__(self, in_channels, hidden_channels, n_blocks=8):
        super().__init__()
        self.n_blocks= n_blocks

        self.blocks = nn.ModuleList([
            [ResNetBlock(in_channels, hidden_channels)] + # first block
            [ResNetBlock(hidden_channels, hidden_channels) for _ in range(self.n_blocks - 2)] + # All blocks that aren't first or last are 'middle blocks'
